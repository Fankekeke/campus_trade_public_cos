...
public class PharmacyInfoController {
    ...
    private final IGoodsOrderInfoService goodsOrderInfoService;
    ...
    public R homeData(@RequestParam("id") Integer id) {
        return R.ok(pharmacyInfoService.homeData(id));
    }
    ...
    public R queryDetail(@RequestParam("pharmacyId") Integer pharmacyId) {
        ...
        // 查询订单信息
        result.put("order", goodsOrderInfoService.list(Wrappers.<GoodsOrderInfo>lambdaQuery().eq(GoodsOrderInfo::getUserId, pharmacyInfo.getId())));
        return R.ok(result);
    }
