...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    @Autowired
    private IPharmacyInfoService pharmacyInfoService;
     ...
     */
    @Override
    public void registPharmacy(String username, String password, PharmacyInfo pharmacyInfo) throws Exception {
        User user = new User();
        user.setPassword(MD5Util.encrypt(username, password));
        user.setUsername(username);
        user.setCreateTime(new Date());
        ...
        pharmacyInfo.setUserId(Math.toIntExact(user.getUserId()));
        pharmacyInfo.setCreateDate(DateUtil.formatDateTime(new Date()));
        pharmacyInfoService.save(pharmacyInfo);
