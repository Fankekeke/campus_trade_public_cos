...
export default {
  ...
  methods: {
    ...
    getDrug () {
      this.$get('/stock/drug-info/queryGoodsByPharmacyId', {pharmacyId: this.currentUser.userId}).then((r) => {
        this.drugList = r.data.data
      })
    },
    getPharmacy () {
      this.$get('/stock/pharmacy-info/list').then((r) => {
    ...
    handleSubmit () {
      this.form.validateFields((err, values) => {
        ...
        if (!err) {
          this.loading = true
          this.$post('/stock/pharmacy-inventory/batch/put', {
            pharmacyId: this.pharmacyInfo.id,
